...
def detect_images(model: YOLOModel, video_path: str, output_json: str, verbose: bool = False):
    all_detections = []
    batch_count = 0
    total_frames = 0
    for batch in extract_frames(video_path):
        ...
        if verbose:
            print(f"Dumping detections to {output_json}...")
            batch_count += 1
...
def segment_images(model: YOLOModel, video_path: str, output_json: str, verbose: bool = False):
    all_segmentations = []
    batch_count = 0
    total_frames = 0
    for batch in extract_frames(video_path):
        batch_size = len(batch)
        if verbose:
            print(f"Dumping segmentations to {output_json}...")
            batch_count += 1
            total_frames += batch_size
        segmentations = model.segment_batch(batch, output_json)
    ...
    print(f"Segmentations saved to {output_json}")    
...
def main():
    ...
    if args.task == "detect":
        detect_images(model, args.video_path, args.output_json, verbose=args.verbose)
    elif args.task == "segment":
        segment_images(model, args.video_path, args.output_json, verbose=args.verbose)
